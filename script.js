...
const schemaConfig = {
    ...
    phieukho: {
        ...
        fields: [{ key: 'MaPhieu', label: 'Mã Phiếu', type: 'text' }, { key: 'NgayLap', label: 'Ngày Lập', type: 'date' }, { key: 'LoaiPhieu', label: 'Loại (Nhập/Xuất)', type: 'text' }, { key: 'MaNV', label: 'Người Lập', type: 'select', sourceEndpoint: '/nhanvien', valueKey: 'MaNV', labelKey: 'HoTen' }, { key: 'MaDD', label: 'Địa Điểm', type: 'select', sourceEndpoint: '/diadiem', valueKey: 'MaDD', labelKey: 'Ten' }]
    },
    khuvuc: {
...
async function loadEntity(key) {
    ...
    try {
        ...
        config.fields.forEach(f => tableHTML += `<th>${f.label}</th>`);
        ...
        if (data.length === 0) {
            tableHTML += `<tr><td colspan="${config.fields.length + (hasAction ? 1 : 0)}" style="text-align:center; color:#999; padding:20px;">Chưa có dữ liệu nào.</td></tr>`;
        } else {
            data.forEach(item => {
                tableHTML += `<tr>`;
    ...
    try {
        ...
        document.getElementById('table-container').innerHTML = `
                ...
                <div class="charts-container">
                    <div class="chart-box"><h4>Doanh Thu Theo Địa Điểm</h4><canvas id="branchChart"></canvas></div>
...
async function showModal(isAdd = true, encodedData = null) {
    ...
    for (let i = 0; i < config.fields.length; i++) {
        const f = config.fields[i]; const isPK = (f.key === config.primaryKey);
        ...
        } else { htmlInputs += `<input id="swal-${f.key}" type="${f.type}" class="swal2-input" placeholder="${f.label}" value="${val}" ${inputState}>`; }
    }
    Swal.close();
...
async function printPDF(maPhieu) {
    try {
        ...
        printWindow.document.body.innerHTML = `<div class="header"><h1>PHIẾU ${p.LoaiPhieu}</h1><p>Mã: ${p.MaPhieu} | Ngày lập: ${timeFmt}</p></div><p><b>Người lập:</b> ${p.NguoiLap}</p><p><b>Kho:</b> ${p.TenKho}</p><table><thead><tr><th>STT</th><th>Sản phẩm</th><th>Số lượng</th></tr></thead><tbody>${data.map((item, i) => `<tr><td>${i + 1}</td><td>${item.TenSP}</td><td>${item.SoLuongNhap}</td></tr>`).join('')}</tbody></table><div class="footer"><div>Người lập</div><div>Thủ kho</div><div>Quản lý</div></div>`;
